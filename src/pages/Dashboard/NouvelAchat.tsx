...
function NouvelAchat() {
  ...
  const [openedFournisseurModal, { open: openFournisseurModal, close: closeFournisseurModal }] = useDisclosure(false);
  ...
  const { data: fournisseurs, isLoading: isLoadingFournisseur } = useQuery({
    queryKey: keyFournisseur,
    queryFn: () => fournisseurService.getByUser(session!.user.id),
    enabled: !!session
  });

  const keyDepot = ['depots', session!.user.id];
  const { data: depots, isLoading: isLoadingDepot } = useQuery({
    queryKey: keyDepot,
    queryFn: () => depotService.getByUser(session!.user.id),
    enabled: !!session
  });
  ...
  const { mutate: createAchat, isPending: loadingCreate } = useMutation({
    mutationFn: (data: any) => achatService.create(data),
    onSuccess: (data) => {
      ...
      navigate(`/dashboard/approvisionnements/${data._id}`);
    }
  });

  const { mutate: createFournisseur, isPending: loadingCreateFournisseur } = useMutation({
    mutationFn: (data: any) => fournisseurService.create(data),
    onSuccess: () => {
      closeFournisseurModal();
      qc.invalidateQueries({ queryKey: keyFournisseur });
      formF.reset();
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      // Ctrl+Enter pour valider l'achat
      ...
      if (event.key === 'Escape') {
        navigate('/dashboard/approvisionnements');
      }
    };
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900 min-h-screen">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button
                ...
                onClick={() => navigate('/dashboard/approvisionnements')}
              >
                Retour
              </Button>
              <div>
        ...
        <form onSubmit={form.onSubmit(onCreate)}>
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-4">
              ...
              {/* Récapitulatif */}
              <Paper shadow="sm" p="md" radius="md" className="bg-gradient-to-br from-blue-500 to-blue-600 text-white">
              ...
              <Paper shadow="sm" p="md" radius="md" className="bg-white dark:bg-slate-800">
                ...
                <div ref={bodyRef}>
                  <Table striped highlightOnHover withTableBorder withColumnBorders>
                    <Table.Thead className="bg-gradient-to-r from-blue-500 to-blue-600">
                      <Table.Tr>
                    ...
                    <Table.Tbody>
                      {form.getValues().produits.map((item: any, index: number) => (
                        <Table.Tr key={index}>
                          ...
                          <Table.Td>
                            <Text fw={700} className="text-blue-600 dark:text-blue-400 text-right">
      ...
      <Modal
        ...
        centered
      >
        <form onSubmit={formF.onSubmit(onCreateF)}>
          <Stack gap="md">
            <TextInput
              label="Nom du fournisseur"
              placeholder="Entrez le nom"
              required
              {...formF.getInputProps('nom')}
            />
            <TextInput
              label="Téléphone"
              placeholder="Entrez le numéro"
              {...formF.getInputProps('tel')}
            />
            <TextInput
              label="Adresse"
              placeholder="Entrez l'adresse"
              {...formF.getInputProps('addr')}
            />
            <Group justify="flex-end" mt="md">
              <Button variant="subtle" onClick={closeFournisseurModal}>
                Annuler
              </Button>
              <Button type="submit" color="blue" loading={loadingCreateFournisseur}>
